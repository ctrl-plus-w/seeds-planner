...
export function ParetoPlot({ solutions, selectedRank, onSelect }: ParetoPlotProps) {
  ...
  useEffect(() => {
    ...
    const traces: Plotly.Data[] = [
      {
        ...
        marker: { size: 6, color: "#0ea5e9", opacity: 0.75 },
        hovertemplate:
          "#%{text}<br>Compat: %{x:.2f}<br>Espace: %{y:.0f}%<extra></extra>",
      ...
      },
      {
        ...
        marker: { size: 10, color: "#16a34a", opacity: 1 },
        hovertemplate:
          "#%{text}<br>Compat: %{x:.2f}<br>Espace: %{y:.0f}%<extra></extra>",
    ...
    const layout: Partial<Plotly.Layout> = {
      xaxis: { title: { text: "Compatibilite" } },
      yaxis: { title: { text: "Espace utilise (%)" } },
